...
class Neural_Network:
    ...
    def __init__(self, n_labels, channels, input_shape=None, architecture=None,
        ...
        #self.hidden_model = Model(inputs=model.input, outputs=model.get_layer("relu").output)
   ...
   # Training the Neural Network model
    ...
    def load(self, file_path, custom_objects={}):
        ...
        self.model.compile(optimizer=Adam(learning_rate=self.learning_rate),
                           loss=self.loss, metrics=self.metrics)
    #---------------------------------------------------#
    #     Customized function by Chiba University of Medicine             #
    #--------------------------------------------------#
    # model summary
    def summary(self):
    ...
    # Get actmodel 
    def actmodel(self):
        self.actmodel = models.Model(inputs = self.model.input, outputs=[self.model.get_layer(name).output for name in ["top_activation","avg_pool", "dropout_2"]])
        return self.actmodel
    # Get actmodel summary
    def actmodel_summary(self):
        print(self.actmodel.summary())
        return
    # Get actmodel prediction
    def actmodel_predict(self, prediction_generator):
        # Run inference process with the Keras predict function
        preds = self.actmodel.predict(prediction_generator, workers=self.workers,
                                   max_queue_size=self.batch_queue_size,
                                   use_multiprocessing=self.multiprocessing,
                                   verbose=self.verbose)
        # Output predictions results
