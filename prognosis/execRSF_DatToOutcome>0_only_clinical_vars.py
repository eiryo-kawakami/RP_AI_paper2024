...
for i in range(n_rep):
    ...
    model = RandomSurvivalForest(n_estimators=2000,min_samples_split=10,min_samples_leaf=15,max_features="sqrt",oob_score=True,n_jobs=16,verbose=1,random_state=i)
    model.fit(X_train, yy_train)
    with open("./RSF_models/RP_eyesight_loss<0.3_RSF_DaysToOutcome>0_BGalone_rep"+str(i+1)+".sav", 'wb') as f:
        cPickle.dump(model, f)

    cindex_train.append(model.score(X_train, yy_train))
    cindex_test.append(model.score(X_test, yy_test))

    perm = permutation_importance(model, X_train, yy_train, n_repeats=10, random_state=i)
...
cindex_summary.to_csv("RP_eyesight_loss<0.3_RSF_DaysToOutcome>0_BGalone_cindex.txt",sep="\t",index=False)
...
imp_summary.to_csv("RP_eyesight_loss<0.3_RSF_DaysToOutcome>0_BGalone_varimp.txt",sep="\t")
