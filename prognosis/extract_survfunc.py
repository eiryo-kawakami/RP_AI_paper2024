...
X = dat_merged.drop(["patient_id","studynumber","eyesightloss<0.3","DaysToOutcome"],axis=1)
...
for i in range(10):
	...
	test_chf.to_csv("RP_eyesight_loss<0.3_RSF_DaysToOutcome>0_top10vars_block_split_test+already_chf_rep"+str(i+1)+".txt",sep="\t",index=False)
